...
startGenIconssets = (setting)->
	...
	File.findOne( relation: 'setting:'+setting._id, key: "background" ).exec (err, file)->
    if err then throw err
		background =  file.name
		File.findOne( relation: 'setting:'+setting._id, key: "logo" ).exec (err, file)->
      if err then throw err
			logo =  file.name
			File.findOne( relation: 'setting:'+setting._id, key: "icon" ).exec (err, file)->
				if err then throw err
				icon =  file.name
	...
	createIcons = (format)->
		if format=="icon" then key="icon" else key="logo"
		if key==="logo" then size={width:1024,height:768} else size={width:286,height:286}
		targetImageLink = ""
		image = gm()
		filetype = "png"
