...
module.exports.generate = function(magazine) {
	...
	File.find({relation: 'magazine:'+magazine._id}).execFind(function(err, files){
		...
		Page.find({magazine: magazine._id}).exec(function(err, pages){
			...
			Article.find({_id: { $in: articleIds}}).execFind(function(err, articles){
				var newarticles = {};
		...
		Page.find({magazine: magazine._id}).exec(function(err, pages){
			...
			_.each(pages, function(page){
				...
				Article.find({_id: page.article}).execFind(function(err, article){
					if (err) { return console.log(err, "no article found in pages with id", page.article); }
					article = article.pop();

					File.find({ 'relation': 'article:'+article._id}).execFind(function(err, files){
